...
class ProfileForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if state and state.isdigit():
            self.fields['city'] = forms.ModelChoiceField(queryset=State.objects.get(id=int(state)).city.all(),
                                                         empty_label='Select a city')
        else:
            self.fields['city'] = forms.ChoiceField(choices=[('', 'Select a city')])

        if city and city.isdigit():
            self.fields['pincode'] = forms.ModelChoiceField(queryset=City.objects.get(id=int(city)).pincode.all(),
                                                            empty_label='Select a pincode')
        else:
